    ...
    public class ADTFile
    {
        ...
        /*private void parseFile(MPQFile mpqfile)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                {
                    int index = i * 16 + j;
                    BoogieCore.Log(LogType.FileDebug, "Parsing MCNK Chunk #{0} [{1}, {2}]", index, i, j);
                    mapChunkTable[i][j] = parseMapChunk(ms, mcin_array[index].MCNK_offset, mcin_array[index].MCNK_size);
                }
            }

            // End of the file !
            BoogieCore.Log(LogType.SystemDebug, "ATDFile: done!", mpqfile.Filename);
        }
        
        // Read in an MCNK chunk at supplied offset.
        private MCNK parseMapChunk(MemoryStream ms, uint offset, uint size)
        {
            BinaryReader bin = new BinaryReader(ms);
            ms.Position = offset;
        ...
        private void parseMapChunkSubChunks(ref MCNK mapChunk, MemoryStream ms, long lastpos)
        {
            BinaryReader bin = new BinaryReader(ms);
